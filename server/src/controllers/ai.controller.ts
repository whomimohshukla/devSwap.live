...
export async function assist(req: Request, res: Response) {
	try {
		const { question, mode, topic, depth, sessionId, includeContext } =
			req.body || {};
		...
		const { answer, meta } = await generateAssistantAnswer({
			question,
			mode,
			topic,
			depth,
...
export async function generateSummary(req: Request, res: Response) {
	try {
		...
		const summary = await generateSessionSummary({
			...
			duration,
			participants: [
				(session.userA as any).name,
				(session.userB as any).name,
			],
		});
