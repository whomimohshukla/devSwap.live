...
import { generateLessonPlan, generateSessionSummary } from "../services/aiService";
...
export async function createLessonPlan(req: Request, res: Response) {
	try {
		...
		const session = await Session.findById(sessionId)
			.populate('userA userB', 'name teachSkills learnSkills skillLevels');
		...
		const teacherLevel = userA.skillLevels?.find((s: any) => s.skillName === session.skillFromA)?.level || 'Intermediate';
		const learnerLevel = userB.skillLevels?.find((s: any) => s.skillName === session.skillFromB)?.level || 'Beginner';
...
export async function getCachedPlans(req: Request, res: Response) {
	try {
		...
		const query = skill ? 
			{ $or: [{ teacherSkill: skill }, { learnerSkill: skill }], cached: true } :
			{ cached: true };
