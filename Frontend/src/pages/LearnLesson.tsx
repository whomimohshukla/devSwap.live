...
function ExampleRunner({ html = '', css = '', js = '' }: { html?: string; css?: string; js?: string }) {
  const srcDoc = useMemo(() => {
    return `<!doctype html><html><head><style>${css}</style></head><body>${html}<script>${js}<\/script></body></html>`;
  }, [html, css, js]);
  return (
    <iframe title="example" className="w-full h-64 rounded border border-[#25282c]" srcDoc={srcDoc} />
  );
}
...
const LearnLesson: React.FC = () => {
  ...
  const score = submitted && selected !== null && lesson.quiz && lesson.quiz.length > 0
    ? (selected === lesson.quiz[0].answerIndex ? 1 : 0)
    : 0;

  return (
    <DocLayout topic={data} currentSlug={lesson.slug}>
      <div className="mb-6">
        <Link className="text-sm text-gray-400 hover:text-teal-400" to={`/learn/${data.id}`}>&larr; {data.title}</Link>
      </div>
      <h1 className="text-3xl font-bold mb-2">{lesson.title}</h1>
      <div className="text-xs text-gray-500 mb-6">{lesson.estMinutes} min • {lesson.level} • {data.title}</div>

      <p className="text-gray-300 mb-6">{lesson.summary}</p>

      <div className="space-y-6">
        {lesson.sections.map((s) => (
          <div key={s.heading} className="rounded border border-[#25282c] bg-[#0f1113] p-4">
      ...
      {lesson.example && (
        <div className="mt-8">
          ...
          <ExampleRunner html={lesson.example.html} css={lesson.example.css} js={lesson.example.js} />
        </div>
      )}
        ...
        <div className="mt-10 rounded border border-[#25282c] bg-[#0f1113] p-5">
          ...
          <div className="space-y-2">
            {lesson.quiz[0].options.map((opt, i) => (
              <label key={i} className={`flex items-center gap-2 cursor-pointer rounded border p-2 ${selected === i ? 'border-teal-500 bg-teal-500/10' : 'border-[#25282c]'}`}>
          ...
          <div className="mt-4 flex items-center gap-4">
            ...
            {submitted && (
              <div className="text-sm">
                ...
                <span className="text-gray-400">{lesson.quiz[0].explanation}</span>
              </div>
            )}
          </div>
        </div>
