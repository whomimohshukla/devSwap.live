...
const Settings: React.FC = () => {
  ...
  const [preferredTimes, setPreferredTimes] = useState<string>('Evenings and weekends');
  ...
  const savePreferences = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPrefsMsg(e?.response?.data?.message || e?.message || 'Failed to save preferences');
    } finally {
  ...
  const handleDelete = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDelError(e?.response?.data?.message || e?.message || 'Failed to delete account');
    } finally {
      setDelLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-gray-800 text-emerald-400"><SlidersHorizontal className="w-4 h-4" /></div>
            <h2 className="text-lg font-semibold text-white">Match & Profile Preferences</h2>
          </div>
          <form onSubmit={savePreferences} className="space-y-4">
            {prefsMsg && (
              <div className={`text-sm ${prefsMsg.includes('saved') ? 'text-emerald-400' : 'text-red-400'}`}>{prefsMsg}</div>
            )}
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white text-sm">Show me in matching</p>
                <p className="text-gray-400 text-xs">If off, you won't appear in match queue.</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" className="sr-only peer" checked={showInMatching} onChange={(e) => setShowInMatching(e.target.checked)} />
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-xs">Allow others to view your profile.</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-300 mb-1">Preferred times</label>
                <input
                  type="text"
            ...
            <button type="submit" disabled={prefsSaving} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm disabled:opacity-60">
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-gray-800 text-emerald-400"><Wifi className="w-4 h-4" /></div>
            <h2 className="text-lg font-semibold text-white">Availability</h2>
          </div>
          <p className="text-gray-400 text-sm mb-4">Control whether other users see you as online and available for matches.</p>
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-gray-800 text-blue-400"><KeyRound className="w-4 h-4" /></div>
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-gray-800 text-indigo-400"><Globe className="w-4 h-4" /></div>
            <h2 className="text-lg font-semibold text-white">Language</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-300 mb-1">App Language</label>
              <select
                value={language}
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-gray-800 text-yellow-400"><Bell className="w-4 h-4" /></div>
            <h2 className="text-lg font-semibold text-white">Notifications</h2>
          </div>
          <div className="space-y-4">
            <label className="flex items-center justify-between">
              <span className="text-sm text-gray-300">Email notifications</span>
              <input type="checkbox" className="scale-110" checked={notifEmail} onChange={(e) => setNotifEmail(e.target.checked)} />
            </label>
            <label className="flex items-center justify-between">
              <span className="text-sm text-gray-300">Product updates</span>
              <input type="checkbox" className="scale-110" checked={notifProduct} onChange={(e) => setNotifProduct(e.target.checked)} />
            </label>
          </div>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-gray-800 text-purple-400"><Shield className="w-4 h-4" /></div>
            <h2 className="text-lg font-semibold text-white">Privacy</h2>
          </div>
          <p className="text-gray-300 text-sm mb-3">
            Manage how we store data in your browser. You can review our policy on the
            <a className="text-indigo-400 hover:text-indigo-300 underline ml-1" href="/privacy">Privacy page</a>.
          </p>
          <div className="flex items-center gap-3">
            <button onClick={reopenCookieDecision} className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg text-sm border border-gray-700">Change cookie decision</button>
            <span className="text-xs text-gray-400">This will reload the page and show the cookie banner again.</span>
          </div>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-gray-800 text-red-400"><Trash2 className="w-4 h-4" /></div>
          ...
          <form onSubmit={handleDelete} className="space-y-3">
            ...
            <p className="text-gray-400 text-sm">Type DELETE to confirm account deletion.</p>
            <input
              type="text"
              value={delConfirm}
