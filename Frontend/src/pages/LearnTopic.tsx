...
import type {
	LearnTopic as LearnTopicType,
	LearnLesson as LearnLessonType,
} from "../lib/learn";
		...
		<DocLayout topic={data}>
			...
			<div className='grid gap-3 sm:grid-cols-2'>
				{data.lessons.map((l: LearnLessonType) => (
					<Link
						key={l.slug}
						to={`/learn/${data.id}/${l.slug}`}
						className='block rounded border border-[#25282c] bg-[#0f1113] p-4 hover:border-teal-500'
					>
						<div className='flex items-center justify-between'>
							<div className='font-semibold text-[15px]'>{l.title}</div>
							<div className='flex items-center gap-2 text-[10px]'>
								<span className='rounded bg-[#0b0c0d] px-2 py-0.5 border border-[#25282c]'>
									{l.estMinutes}m
								</span>
								<span className='rounded bg-[#0b0c0d] px-2 py-0.5 border border-[#25282c] capitalize'>
									{l.level}
								</span>
							</div>
						</div>
