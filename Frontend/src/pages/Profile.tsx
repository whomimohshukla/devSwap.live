	...
	const stats = [
		{
			...
			value: String(
				statsData?.sessionsCompleted ?? statsData?.sessions_completed ?? 0
			),
			icon: BookOpen,
		...
		},
		{
			label: "Skills Taught",
			value: String(
				statsData?.skillsTaught ??
					statsData?.skills_taught ??
		...
		},
		{
			...
			value: String(
				statsData?.skillsLearned ??
					statsData?.skills_learned ??
		...
		{
			label: "Rating",
			value: String(statsData?.rating ?? statsData?.avgRating ?? "—"),
			icon: Star,
								...
								{stats.map((stat, index) => (
									...
									>
										<div className='flex items-center space-x-3'>
											<div
												className={`p-2 rounded-lg bg-gray-800 ${stat.color}`}
											>
