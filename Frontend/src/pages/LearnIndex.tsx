...
const LearnIndex: React.FC = () => {
	...
	const filtered: LearnTopic[] = useMemo(() => {
		...
		return (learnTopics as LearnTopic[]).filter(
			(t) =>
		...
		<div className='min-h-screen bg-[#0b0c0d] text-gray-200'>
			<div className='max-w-6xl mx-auto px-4 py-10'>
				...
				<p className='text-gray-400 mb-6'>
					Browse curated, hands-on tutorials with examples, quizzes, and
					practice tasks. Choose a path and start coding.
				</p>
				...
				<div className='grid gap-4 md:grid-cols-3 mb-8'>
					...
					<div className='rounded border border-[#25282c] bg-[#0f1113] p-4'>
						...
						<ul className='space-y-2 text-sm text-teal-300'>
							...
							<li>
								<Link to='/learn/react' className='hover:underline'>
									Frontend with React
								</Link>
							</li>
							<li>
								<Link to='/learn/nodejs' className='hover:underline'>
									Backend with Node & Express
								</Link>
							</li>
							<li>
								<Link to='/learn/fullstack' className='hover:underline'>
									MERN Full-Stack
								</Link>
							</li>
						</ul>
					...
					{filtered.map((t: LearnTopic) => {
						...
						return (
							<Link
								key={t.id}
								to={`/learn/${t.id}`}
								className='group block rounded-lg border border-[#25282c] bg-[#0f1113] p-5 hover:border-teal-500 transition'
							>
								<div className='flex items-center justify-between'>
									<div className='text-xl font-semibold mb-2 group-hover:text-teal-400'>
										{t.title}
									</div>
									<span className='ml-3 rounded bg-teal-600/20 px-2 py-0.5 text-[10px] text-teal-300 border border-teal-600/30'>
								...
								<div className='mt-4 flex items-center gap-2'>
									<span className='rounded bg-[#0b0c0d] px-2 py-0.5 text-[10px] text-gray-400 border border-[#25282c]'>
										Beginner
									</span>
									<span className='rounded bg-[#0b0c0d] px-2 py-0.5 text-[10px] text-gray-400 border border-[#25282c]'>
										Interactive
								...
								<div className='mt-4'>
									<span className='inline-block rounded bg-teal-600 px-3 py-1.5 text-sm text-black group-hover:bg-teal-500'>
										Start learning →
									</span>
								</div>
							</Link>
