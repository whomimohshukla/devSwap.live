...
const AssistantBar: React.FC = () => {
  ...
  const onSend = async () => {
    ...
    try {
      ...
      addMessage({ id: `${Date.now()}-a`, role: 'assistant', text: answer, t: Date.now(), meta });
      if (typeof meta.cooldownMs === 'number' && meta.cooldownMs > 0) setCooldown(Number(meta.cooldownMs));
      if (meta.model) setModel(String(meta.model));
    } catch (e: any) {
      const msg = e?.response?.data?.message || 'Failed to get answer';
      addMessage({ id: `${Date.now()}-e`, role: 'assistant', text: msg, t: Date.now(), meta: { error: true } });
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      useAssistantStore.setState((s) => ({ cooldownMs: Math.max(0, s.cooldownMs - 1000) }));
    }, 1000);
          ...
          {isOpen && (
            ...
            >
              ...
              <div className="mt-2 text-xs text-gray-500 flex items-center justify-between">
                ...
                {cooldownMs > 0 && (
                  <div>Cooldown active — please wait {Math.ceil(cooldownMs/1000)}s</div>
                )}
              </div>
            </motion.div>
          )}
