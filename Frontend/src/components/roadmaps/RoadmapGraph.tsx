...
export const RoadmapGraph: React.FC<Props> = ({ roadmap }) => {
    ...
    roadmap.stages.forEach((stage: RoadmapStage, sIdx: number) => {
      ...
      stage.steps.forEach((step: RoadmapStep, idx: number) => {
        ...
        if (previousStepNodeId) {
          edges.push({ id: `${previousStepNodeId}->${id}`, source: previousStepNodeId, target: id, animated: false, style: { stroke: '#374151' } });
        } else {
          // Connect stage header to first step
          edges.push({ id: `${stageNodeId}->${id}`, source: stageNodeId, target: id, animated: false, style: { stroke: '#374151' } });
        }
      ...
      if (nextStage && stage.steps.length > 0 && nextStage.steps.length > 0) {
        const lastOfThis = `s${sIdx}-step-${stage.steps[stage.steps.length - 1].id}`;
        const firstOfNext = `s${sIdx + 1}-step-${nextStage.steps[0].id}`;
        edges.push({ id: `${lastOfThis}->${firstOfNext}`, source: lastOfThis, target: firstOfNext, animated: true, style: { stroke: '#10b981' } });
      }
    });
  ...
  const onNodeClick = useCallback((_: any, node: Node) => {
    ...
    const step = stage?.steps.find((st) => `s${sIdx}-step-${st.id}` === node.id);
  ...
  return (
    <div style={{ width: '100%', height: 540 }} className="rounded-lg border border-[#25282c] bg-[#0b0c0d]">
      <ReactFlow nodes={nodes} edges={edges} fitView onNodeClick={onNodeClick}>
        <Background variant={BackgroundVariant.Dots} gap={16} size={1} color="#2a2e33" />
        <MiniMap pannable zoomable nodeColor={() => '#1f2937'} maskColor="rgba(0,0,0,0.4)" />
        <Controls />
      </ReactFlow>
    </div>
  );
