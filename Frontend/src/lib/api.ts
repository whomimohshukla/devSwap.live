...
export const sessionsAPI = {
	...
	createSession: (_data: {
		...
		Promise.reject(
			new Error(
				"Session creation is not supported by the backend. Use matching flow instead."
			)
		),
...
export const matchAPI = {
	...
	acceptMatch: (_matchId: string) =>
		Promise.reject(
			new Error("Accepting a match is not supported by the backend yet.")
		),
...
export const aiAPI = {
	...
	generateLessonPlan: (data: {
		...
		return api.post(`/ai/lesson-plan/${sessionId}`, {
			skill,
			level,
			duration,
		});
...
export const requestsAPI = {
	...
	create: (data: { toUserId: string; message?: string }) =>
