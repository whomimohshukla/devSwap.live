...
export function useRequests() {
	...
	const refreshAll = async () => {
		setLoading(true);
		try {
			const [incPending, incAll, sentPendingRes, sentAll] =
				await Promise.all([
			...
			setIncomingHistory(
				incA.filter((r) => r.status && r.status !== "pending")
			);
			setSentPending(sentP);
			setSentHistory(
				sentA.filter((r) => r.status && r.status !== "pending")
			);
		} finally {
			setLoading(false);
		}
	...
	const accept = async (id: string) => {
		...
		try {
			...
			if (updated) {
				setSentPending((prev) =>
					prev.map((r) =>
						getId(r) === id ? { ...r, status: "accepted" } : r
					)
				);
				setSentHistory((prev) => [
	...
	const decline = async (id: string) => {
		...
		try {
			...
			if (updated) {
				setSentPending((prev) =>
					prev.map((r) =>
						getId(r) === id ? { ...r, status: "declined" } : r
					)
				);
				setSentHistory((prev) => [
	...
	const send = async (toUserId: string, message?: string) => {
		...
		if (created) {
			...
			setSentPending((prev) => [
				created,
				...prev.filter((r) => getId(r) !== getId(created)),
			]);
		} else {
			await refreshAll();
		}
	...
	useEffect(() => {
		...
		const onCreated = (payload: any) => {
			...
			setIncomingPending((prev) => [
				req,
				...prev.filter((r) => getId(r) !== getId(req)),
			]);
			addIncomingNotif({
				requestId: getId(req),
